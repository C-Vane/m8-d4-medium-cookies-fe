...
export default function Reactions({ reviews, postReview, user, editResponse, deleteResponse, claps, postClaps }) {
  ...
  return (
    <>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", color: claps.find((clap) => clap._id === user._id) ? "blue" : "black" }}>
          <Button variant='link' onClick={postClaps}>
          ...
          <Button variant='link' onClick={() => setClaps(!clapsModal)}>
            <span style={{ fontSize: 12, marginLeft: "0.5em" }}>{claps.length} claps</span>
          </Button>
          <FaRegComment
            style={{
              transform: "scale(-1, 1)",
