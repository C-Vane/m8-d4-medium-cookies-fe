...
export default function Reactions({ reviews, postReview, user, editResponse, deleteResponse }) {
    ...
    <>
      ...
      >
        <div style={{ display: "flex", alignItems: "center" }}>
          ...
          />
          <Button variant='link' style={{ fontSize: 12, marginLeft: "0.5em" }} onClick={() => setResponses(!responses)}>
      ...
      {responses && reviews.length > 0 && (
        <>
          <h6>Responses</h6>
          <div style={{ overflowY: "scroll", height: "25vh" }}>
            {reviews.map((response, key) => (
              <ResponsesItem response={response} key={key} editResponse={editResponse} deleteResponse={deleteResponse} user={user} />
            ))}
          </div>
        </>
      )}
